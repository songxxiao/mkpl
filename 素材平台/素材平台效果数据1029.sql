...
select  
     ...
     , mkt_video.name as video_title -- 视频标题
     , cost as consume_money -- 消耗金额
-- 快手
   ...
   --   , play_5s_count
      ...
      , play_end_count / ${strutil.contain(vendor_id!'1', '1') ? "content_impression":"total_play"} as play_end_rate  -- 完整播放率
      , click/${strutil.contain(vendor_id!'1', '1') ? "content_impression":"total_play"} as click_rate -- 点击率
       , valid_play / ${strutil.contain(vendor_id!'1', '1') ? "content_impression":"total_play"}  as valid_play_rate      -- 有效播放率
...
where is_test_data = 0
     and material_video_id in (   select video.id as video_id
...
left join (select id, url, cover_image_url, name  from makepolo.material_market_video) mkt_video on mkt_video.id = creative.material_video_id