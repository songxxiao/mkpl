...
select cr.video_url as video_url
    ...
    , if(cr.video_url='未知', '未知',max(ev.name))   as vendor
    , if(cr.video_url='未知', '未知', max(pro.name)) as project
    , if(cr.video_url='未知', '未知', max(vau.name)) as  video_designer
    , if(cr.video_url='未知', '未知', max(au.name)) as optimizer
    , if(cr.video_url='未知', '未知', max(mcau.name)) as  creator_crew
    ...
    , if(sum(cr.click)=0, 0, sum(cr.convert_count)/sum(cr.click)) as convert_rate  
    , if(sum(cr.convert_count)=0, 0,sum(cr.cost /10000)/sum(cr.convert_count)) as convert_cost  
    , sum(cr.deep_convert_count) as deep_convert_count  
    , if(sum(cr.click)=0, 0,sum(cr.deep_convert_count)/sum(cr.click)) as deep_convert_rate  
    , if(sum(cr.deep_convert_count)=0, 0,sum(cr.cost /10000)/sum(cr.deep_convert_count)) as deep_convert_cost  
    , if(sum(cr.total_play)=0, 0,sum(cr.play_duration_sum)/sum(cr.total_play)) as play_duration_per_play  
    , if(sum(cr.total_play)=0, 0,sum(cr.wifi_play)/sum(cr.total_play)) as wifi_play_rate  
    , sum(total_play) as total_play  
    , sum(valid_play) as valid_play  
    , if(sum(cr.total_play)=0, 0,sum(cr.valid_play)/sum(cr.total_play)) as valid_play_rate 
    , if(sum(cr.valid_play)=0, 0,sum(cr.cost / 10000)/sum(cr.valid_play)) as valid_play_cost 
...
from (
      select date
       ...
       , sum(wifi_play) as wifi_play
         ...
         left join makepolo_common.account_user au on cr.subordinate_account_id = au.id
