...
select video_id as id
      ...
      , t3.cover_image_url
      , t3.name
      , case when t3.is_free=1 then '是' else '否' end as is_free
...
from (
         ...
         from (
                  ...
                  from (
                           ...
                           from account_statement state
                                    left join material_market_order od on state.order_id = od.id
                  ...
                  from (
                           select order_id
                                 ...
                                 , 'now'                                                                             as money_type
                                 , min(
                                 ,         date_format(case when type = ${type_now_str} then date else '2999-12-31' end, '%Y-%m')) as settlement_date
         ...
         left join material_market_order t2 on t1.order_id = t2.id
         left join material_market_video t3 on t3.id = t1.video_id
...
from account_statement state
         left join material_market_order od on state.order_id = od.id
         left join material_market_video mmv on state.video_id = mmv.id
where type = ${type_now_str}
  and state.is_outsource = ${is_outsource}
