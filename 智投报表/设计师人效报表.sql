...
select 
   ...
   , if(content_impression=0, 0, play_end_count / content_impression) as play_end_rate
...
from  (
    ...
    select  account_id  
        ${isNotEmpty(is_dat) && is_dat == '0'  ? ", dat" : ""}
		, sum(cost) as cost
    ...
    from (
        ...
        select account_id
            ${isNotEmpty(is_dat) && is_dat == '0'  ? ", date(create_time) as dat" : ""} 
            , 0 as cost
            , 0 as cost_rebate
            , 0 as impression
