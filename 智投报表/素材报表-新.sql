...
select 
    ...
    , if(video_name='' or video_name is null, '未知', max(video_name)) as video_name
    , if(video_url='' or video_url is null, '未知', max(pro.name)) as project 
    , if(video_url='' or video_url is null, '未知', max(vau.name)) as  video_designer 
    , if(video_url='' or video_url is null, '未知', max(au.name)) as optimizer 
    , if(video_url='' or video_url is null, '未知', max(mcau.name)) as  creator_crew  
